...
model$generate(num_images = 1L,
               diffusion_steps = 250,
               eta = 1.0,
               condition = "blonde hair, heterochromia, blush, long hair, blue eyes, hairband, red eyes, two side up",
               CFG_scale = 7.0) %>% 
  as.array() %>% 
  {.[1,,,]} %>% 
...
model$img2img(tf$expand_dims(img, 0L), 
              ...
              eta = 0.0,
              condition = "expressionless, blush, black hair",
              CFG_scale = 4.0) %>% 
  as.array() %>% 
  {.[1,,,]} %>% 
  EBImage::Image(colormode = "Color") %>% 
...
Spherical_interpolation = function(N){
  ...
  for (initial_noise in x){
    latents <- model$reverse_process(start_x = initial_noise,
                                     ...
                                     diffusion_steps = 20,
                                     eta = 0.0, 
                                     condition = "blue eyes, brown hair, blush, smile, long hair, collarbone, two side up",
                                     CFG_scale = 3.0)
